...
const Slider = ({category}) => {
    const {slider:{nodes}} = useStaticQuery(query);
    ...
    return (
        ...
        <div className="slider-height">
         {nodes.filter(cat => cat.category === category).map((item, itemIndex) => {
            let position = "nextSlide"
        if(itemIndex === index){
          position = "activeSlide"
        }
        if (itemIndex === index - 1 || (index === 0 &&
        itemIndex === item.length -1)) {
         ...
         <button className="prev" aria-label="previous" onClick={()=> setIndex(index -1)}>
        <FiChevronLeft />
      </button>
      <button className="next" aria-label="next" onClick={()=> setIndex(index +1)}>
        <FiChevronRight />
      </button>
      </div>  
      </div>    
